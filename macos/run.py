...
class NetworkScanner:
    ...
    def get_network_range(self):
        try:
            # Get primary interface (en0 for macOS)
    ...
    def get_macos_info(self, ip, username=None, password=None):
        try:
            if username and password:
                ...
                # System info
                stdin, stdout, stderr = ssh.exec_command('hostname && sw_vers && who')
                hostname = stdout.readline().strip()
                sw_vers = stdout.read().decode()
                who = stdout.readline().strip()

                # Services info using launchctl
                ...
                for line in stdout:
                    if line.startswith('Name:'):
                        share_info = ShareInfo(
                            ...
                            path='/Volumes',  # Default macOS share path
    ...
    def get_device_details(self, ip, username=None, password=None):
        ...
        try:
            ...
            # MAC address and vendor using arp on macOS
